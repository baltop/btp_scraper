# -*- coding: utf-8 -*-
"""
Chuncheoncci(춘천상공회의소) 스크래퍼 - Enhanced 버전
"""
...
class EnhancedChuncheoncciScraper(StandardTableScraper):
    """춘천상공회의소 공지사항 스크래퍼 - 향상된 버전"""
    
    def __init__(self):
        super().__init__()
        self.base_url = "http://chuncheoncci.korcham.net"
        self.list_url = "http://chuncheoncci.korcham.net/front/board/boardContentsListPage.do?boardId=10518&menuId=757"
        
        # Chuncheoncci 특화 설정
        self.verify_ssl = False  # HTTP 사이트이므로 SSL 검증 안함
        self.default_encoding = 'utf-8'
        self.timeout = 30
        self.delay_between_requests = 1  # 더 빠른 실행을 위해 1초로 설정
        
        # JavaScript 기반 상세 페이지 접근을 위한 기본 URL
        self.detail_base_url = "http://chuncheoncci.korcham.net/front/board/boardContentsView.do"
    ...
    def parse_list_page(self, html_content: str) -> list:
        ...
        for row in data_rows:
            try:
                ...
                # 작성일은 춘천CCI에서는 목록에 없을 수 있음 (상세페이지에서 확인)
                date = ""
                if len(cells) > 2:
...
# 테스트용 함수
def test_chuncheoncci_scraper(pages=3):
    """Chuncheoncci 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedChuncheoncciScraper()
    output_dir = "output/chuncheoncci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"Chuncheoncci 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("Chuncheoncci 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_chuncheoncci_scraper(3)