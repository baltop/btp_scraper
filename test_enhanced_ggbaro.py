...
# -*- coding: utf-8 -*-
"""
경기도 골목상권 상생발전소(ggbaro.kr) Enhanced 스크래퍼 테스트
"""

import os
import sys
import logging
from enhanced_ggbaro_scraper import EnhancedGgbaroScraper
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ggbaro_scraper.log', encoding='utf-8'),
...
def test_ggbaro_scraper(pages=3):
    """ggbaro.kr 스크래퍼 테스트 - 기본 3페이지"""
    print(f"=== 경기도 골목상권 상생발전소 스크래퍼 테스트 시작 (최대 {pages}페이지) ===")
    
    scraper = EnhancedGgbaroScraper()
    output_dir = "output/ggbaro"
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'ggbaro.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='ggbaro.kr 스크래퍼 테스트')
    ...
    try:
        success = test_ggbaro_scraper(pages)
