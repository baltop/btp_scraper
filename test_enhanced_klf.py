#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
한국고용노동교육원(KLF) 스크래퍼 테스트
"""

import os
import sys
import logging
from enhanced_klf_scraper import EnhancedKlfScraper

# 로깅 설정
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.StreamHandler(sys.stdout),
        logging.FileHandler('klf_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_klf_scraper(pages=3):
    """KLF 스크래퍼 테스트 - 기본값 3페이지"""
    output_dir = "output/klf"  # 표준 출력 디렉토리
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"KLF 스크래퍼 테스트 시작 - {pages}페이지")
    
    try:
        scraper = EnhancedKlfScraper()
        scraper.scrape_pages(max_pages=pages, output_base=output_dir)
        
        logger.info("스크래핑 완료")
        
        # 결과 검증
        verify_results(output_dir)
        
    except Exception as e:
        logger.error(f"스크래핑 중 오류 발생: {e}")
        raise

def verify_results(output_dir):
    """결과 검증 - 첨부파일 검증 필수"""
    logger.info("=== 결과 검증 시작 ===")
    
    if not os.path.exists(output_dir):
        logger.error(f"출력 디렉토리가 존재하지 않습니다: {output_dir}")
        return
    
    # 공고 폴더 목록 가져오기
    announcement_folders = [d for d in os.listdir(output_dir) 
                          if os.path.isdir(os.path.join(output_dir, d))]
    
    total_items = len(announcement_folders)
    successful_items = 0
    total_attachments = 0
    korean_files = 0
    file_size_total = 0
    url_check_passed = 0
    wpdm_files = 0
    
    logger.info(f"총 {total_items}개 공고 폴더 발견")
    
    for folder_name in announcement_folders:
        folder_path = os.path.join(output_dir, folder_name)
        
        # content.md 파일 확인
        content_file = os.path.join(folder_path, 'content.md')
        if os.path.exists(content_file):
            successful_items += 1
            
            # 원본 URL 포함 확인
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'klf.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
                logger.warning(f"content.md 읽기 실패 {folder_name}: {e}")
        
        # 첨부파일 검증
        attachments_dir = os.path.join(folder_path, 'attachments')
        if os.path.exists(attachments_dir):
            for filename in os.listdir(attachments_dir):
                total_attachments += 1
                
                # 한글 파일명 검증
                has_korean = any(ord(char) >= 0xAC00 and ord(char) <= 0xD7A3 for char in filename)
                if has_korean:
                    korean_files += 1
                
                # WPDM 관련 파일 확인
                if any(ext in filename.lower() for ext in ['.zip', '.pdf', '.hwp', '.doc']):
                    wpdm_files += 1
                
                # 파일 크기 검증
                att_path = os.path.join(attachments_dir, filename)
                try:
                    file_size = os.path.getsize(att_path)
                    file_size_total += file_size
                    
                    if file_size == 0:
                        logger.warning(f"빈 파일 발견: {filename}")
                    elif file_size < 100:
                        logger.warning(f"의심스럽게 작은 파일: {filename} ({file_size} bytes)")
                    else:
                        logger.debug(f"정상 파일: {filename} ({file_size:,} bytes)")
                        
                except Exception as e:
                    logger.error(f"파일 크기 확인 실패 {filename}: {e}")
    
    # 성공률 계산
    success_rate = (successful_items / total_items) * 100 if total_items > 0 else 0
    url_success_rate = (url_check_passed / successful_items) * 100 if successful_items > 0 else 0
    
    # 결과 요약
    logger.info("=== 검증 결과 요약 ===")
    logger.info(f"총 공고 수: {total_items}")
    logger.info(f"성공적 처리: {successful_items} ({success_rate:.1f}%)")
    logger.info(f"원본 URL 포함: {url_check_passed} ({url_success_rate:.1f}%)")
    logger.info(f"총 첨부파일: {total_attachments}")
    logger.info(f"한글 파일명: {korean_files}")
    logger.info(f"WPDM 파일: {wpdm_files}")
    logger.info(f"총 파일 용량: {file_size_total:,} bytes ({file_size_total/1024/1024:.2f} MB)")
    
    if total_attachments > 0:
        korean_ratio = (korean_files / total_attachments) * 100
        avg_file_size = file_size_total / total_attachments
        wpdm_ratio = (wpdm_files / total_attachments) * 100
        
        logger.info(f"한글 파일명 비율: {korean_ratio:.1f}%")
        logger.info(f"WPDM 파일 비율: {wpdm_ratio:.1f}%")
        logger.info(f"평균 파일 크기: {avg_file_size:,.0f} bytes")
    
    # 품질 평가
    if success_rate >= 90:
        logger.info("✅ 우수한 성과: 90% 이상 성공")
    elif success_rate >= 80:
        logger.info("✅ 양호한 성과: 80% 이상 성공")
    elif success_rate >= 70:
        logger.warning("⚠️ 보통 성과: 70% 이상 성공")
    else:
        logger.error("❌ 개선 필요: 70% 미만 성공")
    
    # WordPress/WPDM 특화 평가
    if total_attachments > 0:
        if korean_ratio >= 50:
            logger.info("✅ 한글 파일명 처리 양호")
        else:
            logger.warning("⚠️ 한글 파일명 처리 개선 필요")
        
        if avg_file_size > 1000:
            logger.info("✅ 파일 다운로드 정상적")
        else:
            logger.warning("⚠️ 파일 크기가 작아 다운로드 문제 가능성")
        
        if wpdm_ratio >= 70:
            logger.info("✅ WPDM 시스템 파일 처리 양호")
        else:
            logger.info("ℹ️ WPDM 외 일반 파일도 포함")
    
    # WordPress 특화 체크
    wordpress_features = 0
    if total_items > 0:
        # WordPress URL 패턴 체크
        sample_folder = announcement_folders[0] if announcement_folders else None
        if sample_folder:
            content_file = os.path.join(output_dir, sample_folder, 'content.md')
            if os.path.exists(content_file):
                with open(content_file, 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read()
                    if 'klf.or.kr/' in content and len([line for line in content.split('\n') if line.strip()]) > 3:
                        wordpress_features += 1
    
    if wordpress_features > 0:
        logger.info("✅ WordPress 구조 정상 감지")
    
    logger.info("=== 검증 완료 ===")
    
    return success_rate >= 80

def test_single_page():
    """단일 페이지 테스트"""
    logger.info("단일 페이지 테스트 시작")
    test_klf_scraper(pages=1)

def test_three_pages():
    """3페이지 테스트"""
    logger.info("3페이지 테스트 시작")
    test_klf_scraper(pages=3)

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='KLF 스크래퍼 테스트')
    parser.add_argument('--pages', type=int, default=3, help='테스트할 페이지 수 (기본값: 3)')
    parser.add_argument('--single', action='store_true', help='단일 페이지만 테스트')
    
    args = parser.parse_args()
    
    if args.single:
        test_single_page()
    else:
        test_klf_scraper(args.pages)