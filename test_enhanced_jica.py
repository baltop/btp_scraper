...
import sys
import logging
from enhanced_jica_scraper import EnhancedJicaScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('jica_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_jica_scraper(pages=3):
    """JICA 스크래퍼 테스트 - 기본값 3페이지"""
    logger.info("=== JICA 스크래퍼 테스트 시작 ===")
    
    # 출력 디렉토리 설정 (표준 규칙)
    output_dir = "output/jica"
    os.makedirs(output_dir, exist_ok=True)
    
    # 스크래퍼 생성 및 실행
    scraper = EnhancedJicaScraper()
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'jica.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='JICA 스크래퍼 테스트')
