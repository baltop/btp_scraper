...
# -*- coding: utf-8 -*-
"""
광주테크노파크(GJTP) Enhanced 스크래퍼 테스트 스크립트
"""

import os
import sys
import logging
from enhanced_gjtp_scraper import EnhancedGJTPScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('gjtp_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_gjtp_scraper(pages=3):
    """GJTP 스크래퍼 테스트"""
    logger.info("=== GJTP Enhanced 스크래퍼 테스트 시작 ===")
    
    # 스크래퍼 초기화
    scraper = EnhancedGJTPScraper()
    
    # 출력 디렉토리 설정 (표준 패턴)
    output_dir = "output/gjtp"
    os.makedirs(output_dir, exist_ok=True)
    
    try:
        # 스크래핑 실행
        logger.info(f"GJTP 스크래핑 시작 - 최대 {pages}페이지")
        scraper.scrape_pages(max_pages=pages, output_base=output_dir)
        
        logger.info("=== GJTP 스크래핑 완료 ===")
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    ...
                    # 원본 URL 포함 확인
                    if '**원본 URL**:' in content and 'gjtp.or.kr' in content:
                        url_check_passed += 1
                    
                    # 카테고리 정보 확인 (GJTP는 카테고리가 없을 수 있음)
                    if '**카테고리**:' in content:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='GJTP Enhanced 스크래퍼 테스트')
