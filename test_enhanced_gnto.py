...
# -*- coding: utf-8 -*-
"""
경남관광재단(gnto.or.kr) Enhanced 스크래퍼 테스트
"""

import os
import sys
import logging
from enhanced_gnto_scraper import EnhancedGNTOScraper
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gnto_scraper.log', encoding='utf-8'),
...
def test_gnto_scraper(pages=3):
    """경남관광재단 스크래퍼 테스트 - 기본 3페이지"""
    print(f"=== 경남관광재단 스크래퍼 테스트 시작 (최대 {pages}페이지) ===")
    
    scraper = EnhancedGNTOScraper()
    output_dir = "output/gnto"
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'gnto.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='경남관광재단 스크래퍼 테스트')
    ...
    try:
        success = test_gnto_scraper(pages)
