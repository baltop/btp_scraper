...
# -*- coding: utf-8 -*-
"""
HT Dream Enhanced 스크래퍼 테스트 스크립트
"""

import os
import sys
import logging
from enhanced_htdream_scraper import EnhancedHtdreamScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('htdream_test.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_htdream_scraper(pages=3):
    """HT Dream 스크래퍼 테스트"""
    logger.info("=== HT Dream Enhanced 스크래퍼 테스트 시작 ===")
    
    # 출력 디렉토리 설정
    output_dir = "output/htdream"
    os.makedirs(output_dir, exist_ok=True)
    
    # 스크래퍼 초기화
    scraper = EnhancedHtdreamScraper()
    ...
    for folder_name in announcement_folders:
        ...
        if os.path.exists(content_file):
            ...
            try:
                ...
                # 원본 URL 포함 확인
                if '**원본 URL**:' in content and 'htdream.kr' in content:
                    url_check_passed += 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='HT Dream 스크래퍼 테스트')
    ...
    test_htdream_scraper(pages=test_pages)