...
# -*- coding: utf-8 -*-
"""
경기도기술개발원(GDTP) Enhanced 스크래퍼 테스트 스크립트
"""
...
import logging
import argparse
from enhanced_gdtp_scraper import EnhancedGDTPScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('gdtp_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_gdtp_scraper(pages=3):
    """GDTP 스크래퍼 테스트"""
    logger.info("=" * 60)
    logger.info("GDTP Enhanced 스크래퍼 테스트 시작")
    logger.info("=" * 60)
...
def verify_results(output_dir):
    ...
    # 기술개발원 관련 키워드 (검증용)
    gdtp_keywords = [
        '기술개발', '연구개발', 'R&D', '창업', '스타트업', '벤처',
        '지원사업', '공모', '모집', '교육', '컨설팅', '멘토링',
        '경기도', '기업', '산업', '혁신', '기술사업화', '창작',
        '인큐베이팅', '액셀러레이팅', '네트워킹', '스튜디오'
    ]
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    ...
                    # 원본 URL 포함 확인
                    if '**원본 URL**:' in content and 'gdtp.or.kr' in content:
                        url_check_passed += 1
                    
                    # 기술개발원 관련 키워드 확인
                    found_keywords = [kw for kw in gdtp_keywords if kw in content]
                    if found_keywords:
    ...
    if total_attachments > 0:
        ...
        logger.info("ℹ️  첨부파일이 있는 공고가 없습니다")

def main():
    parser = argparse.ArgumentParser(description='GDTP Enhanced 스크래퍼 테스트')
