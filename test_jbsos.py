#!/usr/bin/env python3
"""
JBSOS Enhanced 스크래퍼 테스트
전북특별자치도 사회적경제지원센터 스크래퍼 테스트 실행
"""
...
def test_jbsos_scraper(pages=3):
    """JBSOS 스크래퍼 테스트 - 3페이지"""
    logger.info("=== Enhanced JBSOS 스크래퍼 테스트 시작 ===")
        ...
        for page_num in range(1, pages + 1):
            ...
            for i, announcement in enumerate(announcements):
                try:
                    ...
                    # content.md 파일 생성
                    content_md = f"# {title}\n\n"
                    content_md += f"**작성자**: {announcement.get('author', '')}\n"
                    content_md += f"**작성일**: {announcement.get('date', '')}\n"
                    content_md += f"**조회수**: {announcement.get('views', '')}\n"
                    if announcement.get('is_notice'):
                        content_md += f"**구분**: 공지사항\n"
...
if __name__ == "__main__":
    success = test_jbsos_scraper(pages=3)
