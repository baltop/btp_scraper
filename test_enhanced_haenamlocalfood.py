...
# -*- coding: utf-8 -*-
"""
해남로컬푸드(haenamlocalfood.kr) Enhanced 스크래퍼 테스트
"""

import os
import sys
import logging
from enhanced_haenamlocalfood_scraper import EnhancedHaenamLocalFoodScraper
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('haenamlocalfood_scraper.log', encoding='utf-8'),
...
def test_haenamlocalfood_scraper(pages=3):
    """해남로컬푸드 스크래퍼 테스트 - 기본 3페이지"""
    print(f"=== 해남로컬푸드 스크래퍼 테스트 시작 (최대 {pages}페이지) ===")
    
    scraper = EnhancedHaenamLocalFoodScraper()
    output_dir = "output/haenamlocalfood"
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'haenamlocalfood.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='해남로컬푸드 스크래퍼 테스트')
    ...
    try:
        success = test_haenamlocalfood_scraper(pages)
