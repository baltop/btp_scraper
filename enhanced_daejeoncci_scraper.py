# -*- coding: utf-8 -*-
"""
DaejeonCCI(대전상공회의소) 스크래퍼 - Enhanced 버전
"""
...
class EnhancedDaejeonCCIScraper(StandardTableScraper):
    """대전상공회의소 공지사항 스크래퍼 - 향상된 버전"""
...
# 테스트용 함수
def test_daejeoncci_scraper(pages=3):
    """DaejeonCCI 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedDaejeonCCIScraper()
    output_dir = "output/daejeoncci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"DaejeonCCI 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("DaejeonCCI 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_daejeoncci_scraper(3)