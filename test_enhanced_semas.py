...
def test_semas_scraper(pages=3):
    """SEMAS 스크래퍼 테스트"""
    print("=" * 60)
    print("SEMAS 향상된 스크래퍼 테스트 시작")
    print("=" * 60)
    ...
    try:
        ...
        print(f"📄 게시판 URL: {scraper.list_url}")
        print()
    ...
    try:
        ...
        for i, ann_dir in enumerate(sorted(announcement_dirs), 1):
            try:
                ...
                if has_content and file_count == 0:
                    # 첨부파일이 없는 경우는 정상
                    status = "✅"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SEMAS 스크래퍼 테스트")
    ...
    success = test_semas_scraper(pages)
