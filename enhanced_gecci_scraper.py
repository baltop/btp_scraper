# -*- coding: utf-8 -*-
"""
GECCI(경기동부상공회의소) 스크래퍼 - Enhanced 버전
"""
...
class EnhancedGECCIScraper(StandardTableScraper):
    """경기동부상공회의소 공지사항 스크래퍼 - 향상된 버전"""
    
    def __init__(self):
        super().__init__()
        self.base_url = "https://gecci.korcham.net"
        self.list_url = "https://gecci.korcham.net/front/board/boardContentsListPage.do?boardId=10756&menuId=1816"
        ...
        self.detail_base_url = "https://gecci.korcham.net/front/board/boardContentsView.do"
    ...
    def parse_list_page(self, html_content: str) -> list:
        ...
        for row in rows:
            ...
            if len(cells) >= 2:  # GECCI는 번호, 제목 컬럼만 있음
                # th가 있는 헤더 행은 제외
                if not row.find('th'):
        ...
        for row in data_rows:
            try:
                ...
                content_id = None
                if onclick:
                    # javascript:contentsView('117493') 형태에서 ID 추출
                ...
                # 작성일은 GECCI에서는 목록에 없음 (상세페이지에서 확인)
                date = ""
...
# 테스트용 함수
def test_gecci_scraper(pages=3):
    """GECCI 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedGECCIScraper()
    output_dir = "output/gecci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"GECCI 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("GECCI 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_gecci_scraper(3)