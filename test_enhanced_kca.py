...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('kca_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_kca_scraper(pages=3):
    """KCA 스크래퍼 테스트"""
    logger.info("=== Enhanced KCA 스크래퍼 테스트 시작 ===")
    ...
    try:
            ...
            if detail_response:
                ...
                if detail_info['attachments']:
                    ...
                    for i, att in enumerate(detail_info['attachments'], 1):
                        ...
                        if att['size']:
                            logger.info(f"     크기: {att['size']}")
                        if att['download_count']:
                            logger.info(f"     다운로드: {att['download_count']}회")
                ...
                with open(content_path, 'w', encoding='utf-8') as f:
                    ...
                    f.write(f"**분류**: {test_ann.get('category', '')}\n\n")
                    f.write(f"**작성자**: {test_ann.get('writer', '')}\n\n")
        ...
        for folder in result_folders:
            ...
            if os.path.exists(content_file):
                ...
                # KCA 관련 내용이 있는지 확인
                if 'kca.kr' in content or '한국방송통신전파진흥원' in content or 'KCA' in content or '방송통신' in content:
                    successful_items += 1
...
def verify_results(output_dir="output/kca"):
    """결과 검증"""
    logger.info("=== 결과 검증 ===")
    ...
    for folder in folders:
        ...
        if os.path.exists(content_file):
            ...
            # 원본 URL 포함 확인
            if '**원본 URL**:' in content and 'kca.kr' in content:
                url_check_passed += 1
...
if __name__ == "__main__":
    ...
    success = test_kca_scraper(pages)
    
    if success:
        print("\n✓ Enhanced KCA 스크래퍼 테스트 성공!")
        verify_results()
    else:
        print("\n✗ Enhanced KCA 스크래퍼 테스트 실패!")
        sys.exit(1)