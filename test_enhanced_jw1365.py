...
import logging
import argparse
from enhanced_jw1365_scraper import EnhancedJW1365Scraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('jw1365_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_jw1365_scraper(pages=3):
    """1365 자원봉사포털 스크래퍼 테스트 - 기본값 3페이지"""
    scraper = EnhancedJW1365Scraper()
    output_dir = "output/jw1365"  # 표준 출력 디렉토리
    
    # 출력 디렉토리 생성
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"=== 1365 자원봉사포털 스크래퍼 테스트 시작 ({pages}페이지) ===")
...
def verify_results(output_dir):
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and '1365.go.kr' in content:
                        url_check_passed += 1
            except Exception as e:
    ...
    # 특별 검증: 1365 사이트 특성상 이미지가 많을 것으로 예상
    if image_files > 0:
        logger.info(f"✅ 이미지 파일 {image_files}개 발견 - 1365 사이트 특성 확인")
...
def main():
    parser = argparse.ArgumentParser(description='1365 자원봉사포털 Enhanced 스크래퍼 테스트')
    parser.add_argument('--pages', type=int, default=3, help='스크래핑할 페이지 수 (기본값: 3)')
    parser.add_argument('--output', default='output/jw1365', help='출력 디렉토리 (기본값: output/jw1365)')
    ...
    if args.verify_only:
        ...
        test_jw1365_scraper(args.pages)
