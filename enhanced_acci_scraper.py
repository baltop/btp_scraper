...
class EnhancedACCIScraper(StandardTableScraper):
    """안양상공회의소 공지사항 스크래퍼 - 향상된 버전"""
...
def test_acci_scraper(pages=3):
    ...
    scraper = EnhancedACCIScraper()
    output_dir = "output/acci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"ACCI 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("ACCI 스크래퍼 테스트 완료")
