# -*- coding: utf-8 -*-
"""
YeongcheonCCI(영천상공회의소) 스크래퍼 - Enhanced 버전
"""
...
# 테스트용 함수
def test_yeongcheoncci_scraper(pages=3):
    """YeongcheonCCI 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedYeongcheonCCIScraper()
    output_dir = "output/yeongcheoncci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"YeongcheonCCI 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("YeongcheonCCI 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_yeongcheoncci_scraper(3)