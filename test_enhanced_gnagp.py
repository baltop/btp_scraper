...
# -*- coding: utf-8 -*-
"""
경상남도항노화플랫폼(gnagp.com) Enhanced 스크래퍼 테스트
"""

import os
import sys
import logging
from enhanced_gnagp_scraper import EnhancedGNAGPScraper
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gnagp_scraper.log', encoding='utf-8'),
...
def test_gnagp_scraper(pages=3):
    """경상남도항노화플랫폼 스크래퍼 테스트 - 기본 3페이지"""
    print(f"=== 경상남도항노화플랫폼 스크래퍼 테스트 시작 (최대 {pages}페이지) ===")
    
    scraper = EnhancedGNAGPScraper()
    output_dir = "output/gnagp"
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'gnagp.com' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='경상남도항노화플랫폼 스크래퍼 테스트')
    ...
    try:
        success = test_gnagp_scraper(pages)
