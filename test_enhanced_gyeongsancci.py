...
from enhanced_gyeongsancci_scraper import EnhancedGYEONGSANCCIScraper

def test_gyeongsancci_scraper(pages=3):
    """GYEONGSANCCI 스크래퍼 테스트"""
    print("=" * 60)
    print("GYEONGSANCCI 향상된 스크래퍼 테스트 시작")
    print("=" * 60)
    
    try:
        scraper = EnhancedGYEONGSANCCIScraper()
        
        # 출력 디렉토리 설정
        output_dir = "output/gyeongsancci"
...
def verify_file_downloads(output_dir):
    ...
    if os.path.exists(output_dir):
        for root, dirs, files in os.walk(output_dir):
            for file in files:
                ...
                # 특수문자 파일명 체크 (경산상공회의소 특화)
                if any(char in file for char in ['(', ')', '[', ']', '_', '-']):
                    special_char_files.append(file_path)
    ...
    if special_char_files:
        print(f"🔧 특수문자 파일명: {len(special_char_files)}개")
        for file_path in special_char_files[:3]:  # 최대 3개만 표시
            print(f"    - {os.path.basename(file_path)}")
...
def single_page_test():
    """단일 페이지 테스트"""
    print("🔬 단일 페이지 테스트 실행...")
    return test_gyeongsancci_scraper(pages=1)

def full_test():
    """전체 3페이지 테스트"""
    print("🚀 전체 3페이지 테스트 실행...")
    return test_gyeongsancci_scraper(pages=3)

def compare_with_pohangcci():
    """포항상공회의소와 비교 분석"""
    print("🔄 포항상공회의소와 비교 분석:")
    print("-" * 40)
    
    pohangcci_dir = "output/pohangcci"
    gyeongsancci_dir = "output/gyeongsancci"
    
    if os.path.exists(pohangcci_dir) and os.path.exists(gyeongsancci_dir):
        pohangcci_count = len([d for d in os.listdir(pohangcci_dir) if os.path.isdir(os.path.join(pohangcci_dir, d))])
        gyeongsancci_count = len([d for d in os.listdir(gyeongsancci_dir) if os.path.isdir(os.path.join(gyeongsancci_dir, d))])
        
        print(f"  📊 포항상공회의소: {pohangcci_count}개 공고")
        print(f"  📊 경산상공회의소: {gyeongsancci_count}개 공고")
        print(f"  🔗 유사한 사이트 구조로 동일한 스크래퍼 패턴 적용 가능")
    else:
        print("  ℹ️  비교할 데이터가 없습니다")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='GYEONGSANCCI 스크래퍼 테스트')
    ...
    parser.add_argument('--compare', action='store_true', help='포항상공회의소와 비교')
    ...
    if args.compare:
        compare_with_pohangcci()
        exit(0)
    
    if args.single:
        success = single_page_test()
    else:
        success = test_gyeongsancci_scraper(args.pages)
