# -*- coding: utf-8 -*-
"""
Jincci(진주상공회의소) 스크래퍼 - Enhanced 버전
"""
...
class EnhancedJincciScraper(StandardTableScraper):
...
# 테스트용 함수
def test_jincci_scraper(pages=3):
    """Jincci 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedJincciScraper()
    output_dir = "output/jincci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"Jincci 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("Jincci 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_jincci_scraper(3)