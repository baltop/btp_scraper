...
# 스크래퍼 import
from enhanced_ayventure_scraper import EnhancedAyventureScraper
...
def test_ayventure_scraper(pages=3):
    """AYVENTURE 스크래퍼 테스트"""
    print(f"=== AYVENTURE 스크래퍼 테스트 시작 (최대 {pages}페이지) ===")
    
    setup_logging()
    logger = logging.getLogger(__name__)
    
    scraper = EnhancedAyventureScraper()
    output_dir = "output/ayventure"
    ...
    for folder_name in announcement_folders:
        ...
        if os.path.exists(content_file):
            ...
            with open(content_file, 'r', encoding='utf-8') as f:
                content = f.read()
                if '**원본 URL**:' in content and 'ayventure.net' in content:
                    url_check_passed += 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='AYVENTURE 스크래퍼 테스트')
