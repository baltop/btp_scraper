# -*- coding: utf-8 -*-
"""
IcheonCCI(이천상공회의소) 스크래퍼 - Enhanced 버전
"""
...
class EnhancedIcheonCCIScraper(StandardTableScraper):
    """이천상공회의소 공지사항 스크래퍼 - 향상된 버전"""
...
# 테스트용 함수
def test_icheoncci_scraper(pages=3):
    """IcheonCCI 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedIcheonCCIScraper()
    output_dir = "output/icheoncci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"IcheonCCI 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("IcheonCCI 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_icheoncci_scraper(3)