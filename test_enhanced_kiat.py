...
from enhanced_kiat_scraper import EnhancedKiatScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('kiat_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_kiat_scraper(pages=3):
    """KIAT 스크래퍼 테스트 - 기본 3페이지"""
    logger.info("=== Enhanced KIAT 스크래퍼 테스트 시작 ===")
    
    # 출력 디렉토리 설정 (표준 규칙: output/사이트명)
    output_dir = "output/kiat"
    os.makedirs(output_dir, exist_ok=True)
    
    # 스크래퍼 초기화
    scraper = EnhancedKiatScraper()
    # 테스트를 위해 중복 체크 비활성화
    scraper.enable_duplicate_check = False
    
    try:
        # 스크래핑 실행
        logger.info(f"KIAT 사이트 스크래핑 시작 - {pages}페이지")
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'kiat.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced KIAT 스크래퍼 테스트')
