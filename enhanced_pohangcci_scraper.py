...
class EnhancedPOHANGCCIScraper(StandardTableScraper):
    """POHANGCCI 전용 스크래퍼 - Playwright 기반"""
    
    def __init__(self):
        super().__init__()
        
        # POHANGCCI 사이트 기본 정보
        self.base_url = "http://pohangcci.korcham.net"
        self.list_url = "http://pohangcci.korcham.net/front/board/boardContentsListPage.do?boardId=10275&menuId=1440"
        ...
        logger.info("POHANGCCI 스크래퍼 초기화 완료")
    ...
    async def scrape_pages_async(self, max_pages: int = 3, output_base: str = 'output'):
        """비동기 스크래핑 메인 함수"""
        logger.info(f"POHANGCCI 스크래핑 시작: 최대 {max_pages}페이지")
