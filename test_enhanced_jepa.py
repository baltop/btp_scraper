...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('jepa_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_jepa_scraper(pages=3):
    """JEPA 스크래퍼 테스트"""
    logger.info("=== Enhanced JEPA 스크래퍼 테스트 시작 ===")
    ...
    try:
            ...
            if detail_response:
                ...
                if detail_info['attachments']:
                    ...
                    for i, att in enumerate(detail_info['attachments'], 1):
                        logger.info(f"  {i}. {att['name']} - {att['download_url']}")
                        logger.info(f"     타입: {att['download_type']}")
                        if att['bs_idx']:
                            logger.info(f"     공고번호: {att['bs_idx']}")
                        if att['bf_idx']:
                            logger.info(f"     파일번호: {att['bf_idx']}")
                ...
                with open(content_path, 'w', encoding='utf-8') as f:
                    ...
                    f.write(f"**작성자**: {test_ann.get('author', '')}\\n\\n")
                    f.write(f"**작성일**: {test_ann.get('date', '')}\\n\\n")
                    f.write(f"**조회수**: {test_ann.get('views', '')}\\n\\n")
                    f.write(f"**진행상태**: {test_ann.get('status', '')}\\n\\n")
                    f.write("---\\n\\n")
        ...
        for folder in result_folders:
            ...
            if os.path.exists(content_file):
                ...
                # JEPA 관련 내용이 있는지 확인
                if 'jepa.kr' in content or '중소기업일자리경제진흥원' in content or 'JEPA' in content or '일자리경제진흥원' in content:
                    successful_items += 1
        ...
        if total_items > 0:
            ...
            if total_attachments > 0:
                attachment_rate = (downloaded_attachments / total_attachments) * 100
                logger.info(f"첨부파일 다운로드 성공률: {attachment_rate:.1f}% ({downloaded_attachments}/{total_attachments})")
...
def verify_results(output_dir="output/jepa"):
    """결과 검증"""
    logger.info("=== 결과 검증 ===")
    ...
    for folder in folders:
        ...
        if os.path.exists(content_file):
            ...
            # 원본 URL 포함 확인
            if '**원본 URL**:' in content and 'jepa.kr' in content:
                url_check_passed += 1
...
if __name__ == "__main__":
    ...
    success = test_jepa_scraper(pages)
    
    if success:
        print("\\n✓ Enhanced JEPA 스크래퍼 테스트 성공!")
        verify_results()
    else:
        print("\\n✗ Enhanced JEPA 스크래퍼 테스트 실패!")
        sys.exit(1)