...
# -*- coding: utf-8 -*-
"""
한국콘텐츠진흥원(KOCCA) Enhanced 스크래퍼 테스트 스크립트
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('kocca_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_kocca_scraper(pages=3):
    """KOCCA 스크래퍼 테스트"""
    logger.info("=== KOCCA Enhanced 스크래퍼 테스트 시작 ===")
    
    # 스크래퍼 초기화
    scraper = EnhancedKOCCAScraper()
    
    # 출력 디렉토리 설정 (표준 패턴)
    output_dir = "output/kocca"
    os.makedirs(output_dir, exist_ok=True)
    
    try:
        # 스크래핑 실행
        logger.info(f"KOCCA 스크래핑 시작 - 최대 {pages}페이지")
        scraper.scrape_pages(max_pages=pages, output_base=output_dir)
        
        logger.info("=== KOCCA 스크래핑 완료 ===")
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    ...
                    # 원본 URL 포함 확인
                    if '**원본 URL**:' in content and 'kocca.kr' in content:
                        url_check_passed += 1
                    
                    # 카테고리 정보 확인
                    if '**카테고리**:' in content and ('모집공모' in content or '자유공고' in content):
...
def main():
    ...
    parser = argparse.ArgumentParser(description='KOCCA Enhanced 스크래퍼 테스트')
