...
# -*- coding: utf-8 -*-
"""
제주콘텐츠진흥원(JEJUFC) Enhanced 스크래퍼 테스트 스크립트
"""
...
import logging
import argparse
from enhanced_jejufc_scraper import EnhancedJEJUFCScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('jejufc_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_jejufc_scraper(pages=3):
    """JEJUFC 스크래퍼 테스트"""
    logger.info("=" * 60)
    logger.info("JEJUFC Enhanced 스크래퍼 테스트 시작")
    logger.info("=" * 60)
    
    # 출력 디렉토리 설정 (표준 패턴)
    output_dir = "output/jejufc"
    ...
    try:
        ...
        logger.info("=" * 60)
        logger.info("JEJUFC 스크래퍼 테스트 완료")
        logger.info("=" * 60)
...
def verify_results(output_dir):
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    ...
                    # 원본 URL 포함 확인
                    if '**원본 URL**:' in content and 'jejufc.onlinekoreahrd.kr' in content:
                        url_check_passed += 1
                    
                    # 제주콘텐츠진흥원 관련 키워드 확인
                    found_keywords = [kw for kw in jejufc_keywords if kw in content]
                    if found_keywords:
    ...
    if total_attachments > 0:
        ...
        logger.info("ℹ️  이는 PHP 다운로드 스크립트나 권한 문제일 수 있습니다")

def main():
    parser = argparse.ArgumentParser(description='JEJUFC Enhanced 스크래퍼 테스트')
    ...
    try:
        test_jejufc_scraper(pages=pages)
