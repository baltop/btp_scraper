...
from enhanced_jcia_scraper import EnhancedJciaScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('jcia_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_jcia_scraper(pages=3):
    """JCIA 스크래퍼 테스트 - 기본 3페이지"""
    logger.info("=== Enhanced JCIA 스크래퍼 테스트 시작 ===")
    
    # 출력 디렉토리 설정 (표준 규칙: output/사이트명)
    output_dir = "output/jcia"
    os.makedirs(output_dir, exist_ok=True)
    
    # 스크래퍼 초기화
    scraper = EnhancedJciaScraper()
    # 테스트를 위해 중복 체크 비활성화
    scraper.enable_duplicate_check = False
    
    try:
        # 스크래핑 실행
        logger.info(f"JCIA 사이트 스크래핑 시작 - {pages}페이지")
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'jcia.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced JCIA 스크래퍼 테스트')
