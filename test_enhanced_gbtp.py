...
# -*- coding: utf-8 -*-
"""
경북테크노파크(GBTP) Enhanced 스크래퍼 테스트 스크립트
"""
...
from enhanced_gbtp_scraper import EnhancedGbtpScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('gbtp_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_gbtp_scraper(pages=3):
    """경북테크노파크(GBTP) 스크래퍼 테스트"""
    output_dir = "output/gbtp"
    
    logger.info("=" * 50)
    logger.info("경북테크노파크(GBTP) Enhanced 스크래퍼 테스트 시작")
    logger.info("=" * 50)
...
def verify_results(output_dir):
    ...
    # 처리된 제목 파일 확인
    processed_file = os.path.join(output_dir, 'processed_titles_enhancedgbtp.json')
    ...
    for folder in folders[:5]:  # 최대 5개만 상세 검증
        ...
        if os.path.exists(content_file):
            ...
            try:
                ...
                # 원본 URL 포함 확인
                if '**원본 URL**:' in content and 'gbtp.or.kr' in content:
                    url_check_passed += 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='경북테크노파크(GBTP) Enhanced 스크래퍼 테스트')
