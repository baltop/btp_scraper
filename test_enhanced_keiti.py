...
# -*- coding: utf-8 -*-
"""
KEITI Enhanced 스크래퍼 테스트 스크립트
"""

import logging
import os
import sys
from enhanced_keiti_scraper import EnhancedKEITIScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('keiti_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_keiti_scraper(pages=3):
    """KEITI 스크래퍼 테스트 - 기본값 3페이지"""
    logger.info("=== KEITI Enhanced 스크래퍼 테스트 시작 ===")
    
    try:
        # 스크래퍼 초기화
        scraper = EnhancedKEITIScraper()
        
        # 출력 디렉토리 설정 - output/keiti (표준 형식)
        output_dir = "output/keiti"
        os.makedirs(output_dir, exist_ok=True)
        
        # 스크래핑 실행
        scraper.scrape_pages(max_pages=pages, output_base=output_dir)
        
        logger.info("=== KEITI 스크래퍼 테스트 완료 ===")
...
def verify_results(output_dir):
    ...
    for folder_name in announcement_folders[:20]:  # 상위 20개만 검증
        ...
        if os.path.exists(content_file):
            ...
            with open(content_file, 'r', encoding='utf-8') as f:
                content = f.read()
                if '**원본 URL**:' in content and 'keiti.re.kr' in content:
                    url_check_passed += 1
        ...
        if os.path.exists(attachments_dir):
            ...
            for filename in attachment_files:
                ...
                if os.path.exists(att_path):
                    ...
                    elif filename.lower().endswith(('.zip', '.xlsx', '.xls')):
                        # 압축/엑셀 파일 검증
                        logger.debug(f"    기타 파일: {filename}")
    ...
    # KEITI 특화 검증
    logger.info("=== KEITI 특화 검증 ===")
    
    # 환경 관련 키워드 확인
    environment_keywords = ['환경', '에너지', '기후', '탄소', '친환경', '재활용', '오염', '정화', '환경기술', '신재생']
    keyword_count = {}
...
def test_single_page():
    ...
    test_keiti_scraper(pages=1)

def test_three_pages():
    """3페이지 테스트"""
    logger.info("=== 3페이지 테스트 ===")
    test_keiti_scraper(pages=3)

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='KEITI Enhanced 스크래퍼 테스트')
    ...
    if args.single:
        ...
        test_keiti_scraper(pages=args.pages)