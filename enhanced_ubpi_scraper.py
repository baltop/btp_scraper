# -*- coding: utf-8 -*-
"""
UBPI 전용 Enhanced 스크래퍼
사이트: https://www.ubpi.or.kr/sub/?mcode=0403010000
특징: UTF-8 인코딩, 표준 테이블 구조, GET 파라미터 기반 상세 페이지
"""
...
class EnhancedUbpiScraper(StandardTableScraper):
    ...
    def _parse_list_fallback(self, html_content: str) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            try:
                ...
                if len(cells) < 6:  # [공지] [제목] [첨부] [작성자] [조회] [등록일] 최소 6개 컬럼
                    continue
                ...
                # 공지 여부 확인
                is_notice = bool(notice_cell.find('img') or '공지' in notice_cell.get_text())
    ...
    def _parse_detail_fallback(self, html_content: str) -> Dict[str, Any]:
        ...
        if not content_area:
            # 대안: 특정 클래스를 가진 div 찾기
            for div in soup.find_all('div'):
                div_class = div.get('class', [])
                if any('view' in str(cls).lower() or 'content' in str(cls).lower() for cls in div_class):
                    div_text = div.get_text(strip=True)
                    if len(div_text) > 100:
                        content_area = div
                        logger.debug("클래스 기반으로 본문 영역 추정")
                        break
        ...
        try:
            ...
            for pattern in file_patterns:
                ...
                for link in links:
                    ...
                    if href and 'javascript:' not in href:
                        ...
                        if file_url not in seen_urls:
                            ...
                            if not filename or len(filename) < 3:
                                filename = os.path.basename(href) or f"attachment_{len(attachments) + 1}"
                    ...
                    # onclick JavaScript 함수가 있는 경우
                    elif onclick and 'download' in onclick.lower():
                        # JavaScript 함수에서 파일 정보 추출
                        # 예: downloadFile('123', 'filename.pdf')
                        js_match = re.search(r'download\w*\([\'"]([^\'"]+)[\'"](?:,\s*[\'"]([^\'"]+)[\'"])?\)', onclick)
                        if js_match:
                            file_id = js_match.group(1)
                            filename = js_match.group(2) if js_match.group(2) else f"file_{file_id}"
                            
                            # UBPI 다운로드 URL 추정
                            file_url = f"{self.base_url}/common/download.php?file_id={file_id}"
            ...
            # 3. 첨부파일 영역 전용 검색
            attachment_areas = soup.find_all(['div', 'section'], class_=lambda x: x and 
                                           any(word in str(x).lower() for word in ['attach', 'file', '첨부']))
...
# 하위 호환성을 위한 별칭
UbpiScraper = EnhancedUbpiScraper