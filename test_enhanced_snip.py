...
import sys
import logging
from enhanced_snip_scraper import EnhancedSnipScraper
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('snip_scraper.log', encoding='utf-8'),
...
def test_snip_scraper(pages=3):
    """SNIP 스크래퍼 테스트 - 기본값 3페이지"""
    logger.info("SNIP 스크래퍼 테스트 시작")
    
    # 스크래퍼 초기화
    scraper = EnhancedSnipScraper()
    
    # 출력 디렉토리 설정 - 표준 형식
    output_dir = "output/snip"
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'snip.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='SNIP 스크래퍼 테스트')
