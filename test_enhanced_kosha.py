...
import sys
import logging
from enhanced_kosha_scraper import EnhancedKoshaScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('kosha_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_kosha_scraper(pages=3):
    """KOSHA 스크래퍼 테스트 - 기본값 3페이지"""
    output_dir = "output/kosha"  # 표준 출력 디렉토리
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"KOSHA 스크래퍼 테스트 시작 - {pages}페이지")
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'kosha.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def test_single_page():
    ...
    test_kosha_scraper(pages=1)

def test_three_pages():
    """3페이지 테스트"""
    logger.info("3페이지 테스트 시작")
    test_kosha_scraper(pages=3)

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='KOSHA 스크래퍼 테스트')
