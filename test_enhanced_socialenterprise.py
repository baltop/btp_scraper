...
import logging
import argparse
from enhanced_socialenterprise_scraper import EnhancedSocialEnterpriseScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('socialenterprise_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_socialenterprise_scraper(pages=3):
    """한국사회적기업진흥원 스크래퍼 테스트 - 기본값 3페이지"""
    scraper = EnhancedSocialEnterpriseScraper()
    output_dir = "output/socialenterprise"  # 표준 출력 디렉토리
    
    # 출력 디렉토리 생성
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"=== 한국사회적기업진흥원 스크래퍼 테스트 시작 ({pages}페이지) ===")
...
def main():
    parser = argparse.ArgumentParser(description='한국사회적기업진흥원 Enhanced 스크래퍼 테스트')
    parser.add_argument('--pages', type=int, default=3, help='스크래핑할 페이지 수 (기본값: 3)')
    parser.add_argument('--output', default='output/socialenterprise', help='출력 디렉토리 (기본값: output/socialenterprise)')
    ...
    if args.verify_only:
        ...
        test_socialenterprise_scraper(args.pages)
