...
import logging
import argparse
from enhanced_eumseongcci_scraper import EnhancedEumseongcciScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('eumseongcci_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_eumseongcci_scraper(pages=3):
    """음성상공회의소 스크래퍼 테스트 - 기본값 3페이지"""
    scraper = EnhancedEumseongcciScraper()
    output_dir = "output/eumseongcci"  # 표준 출력 디렉토리
    
    # 출력 디렉토리 생성
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"=== 음성상공회의소 스크래퍼 테스트 시작 ({pages}페이지) ===")
...
def verify_results(output_dir):
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'eumseongcci.korcham.net' in content:
                        url_check_passed += 1
            except Exception as e:
    ...
    logger.info(f"이미지 파일: {image_files}")
    ...
    # 특별 검증: 상공회의소 사이트 특성상 이미지 파일이 많을 것으로 예상
    if image_files > 0:
        logger.info(f"✅ 이미지 파일 {image_files}개 발견 - 상공회의소 공지 특성 확인")
    
    if pdf_files > 0:
        logger.info(f"✅ PDF 파일 {pdf_files}개 발견 - 공문서 특성 확인")
...
def main():
    parser = argparse.ArgumentParser(description='음성상공회의소 Enhanced 스크래퍼 테스트')
    parser.add_argument('--pages', type=int, default=3, help='스크래핑할 페이지 수 (기본값: 3)')
    parser.add_argument('--output', default='output/eumseongcci', help='출력 디렉토리 (기본값: output/eumseongcci)')
    ...
    if args.verify_only:
        ...
        test_eumseongcci_scraper(args.pages)
