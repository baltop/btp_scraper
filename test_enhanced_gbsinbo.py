#!/usr/bin/env python3
"""
Enhanced GBSINBO 스크래퍼 테스트 스크립트

사용법:
    python test_enhanced_gbsinbo.py --pages 3
    python test_enhanced_gbsinbo.py --single (1페이지만)
"""
...
import logging
import argparse
from enhanced_gbsinbo_scraper import EnhancedGbsinboScraper
...
def test_gbsinbo_scraper(pages=3):
    """GBSINBO 스크래퍼 테스트"""
    logger.info("=== Enhanced GBSINBO 스크래퍼 테스트 시작 ===")
    
    # 스크래퍼 초기화
    scraper = EnhancedGbsinboScraper()
    
    # 표준 출력 디렉토리 사용
    output_dir = "output/gbsinbo"
...
def verify_results(output_dir):
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'gbsinbo.co.kr' in content:
                        url_check_passed += 1
            except Exception as e:
    ...
    if total_attachments > 0:
        ...
        logger.warning("첨부파일이 발견되지 않았습니다. 사이트 특성을 확인해보세요.")
        ...
        if os.path.exists(content_file):
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    ...
                    if any(keyword in content or keyword in title 
                           for keyword in ['홍보', '캠페인', '봉사', '협약', '업무협약']):
                        announcement_count += 1
                    elif any(keyword in content or keyword in title 
                             for keyword in ['행사', '포럼', '세미나', '교육']):
                        event_count += 1
                    else:
                        notice_count += 1
            except:
    ...
    logger.info(f"  - 홍보/소식: {announcement_count}개 ({announcement_count/total_items*100:.1f}%)")
    logger.info(f"  - 행사/교육: {event_count}개 ({event_count/total_items*100:.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced GBSINBO 스크래퍼 테스트')
