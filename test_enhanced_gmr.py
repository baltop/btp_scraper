...
# -*- coding: utf-8 -*-
"""
경기도시장상권진흥원(gmr.or.kr) Enhanced 스크래퍼 테스트
"""

import os
import sys
import logging
from enhanced_gmr_scraper import EnhancedGMRScraper
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('gmr_scraper.log', encoding='utf-8'),
...
def test_gmr_scraper(pages=3):
    """경기도시장상권진흥원 스크래퍼 테스트 - 기본 3페이지"""
    print(f"=== 경기도시장상권진흥원 스크래퍼 테스트 시작 (최대 {pages}페이지) ===")
    
    scraper = EnhancedGMRScraper()
    output_dir = "output/gmr"
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'gmr.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='경기도시장상권진흥원 스크래퍼 테스트')
    ...
    try:
        success = test_gmr_scraper(pages)
