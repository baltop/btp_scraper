...
# -*- coding: utf-8 -*-
"""
aT수출종합지원시스템(global.at.or.kr) Enhanced 스크래퍼 테스트
"""

import os
import sys
import logging
from enhanced_globalat_scraper import EnhancedGlobalatScraper
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('globalat_scraper.log', encoding='utf-8'),
...
def test_globalat_scraper(pages=3):
    """global.at.or.kr 스크래퍼 테스트 - 기본 3페이지"""
    print(f"=== aT수출종합지원시스템 스크래퍼 테스트 시작 (최대 {pages}페이지) ===")
    
    scraper = EnhancedGlobalatScraper()
    output_dir = "output/globalat"
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'global.at.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='global.at.or.kr 스크래퍼 테스트')
    ...
    try:
        success = test_globalat_scraper(pages)
