...
# -*- coding: utf-8 -*-
"""
한국벤처투자조합협회(KBAN) Enhanced 스크래퍼 테스트
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('kban_scraper.log', encoding='utf-8'),
...
def test_kban_scraper(pages=3):
    """KBAN 스크래퍼 테스트 - 기본 3페이지"""
    print(f"=== KBAN(한국벤처투자조합협회) 스크래퍼 테스트 시작 (최대 {pages}페이지) ===")
    
    scraper = EnhancedKBANScraper()
    output_dir = "output/kban"
...
def verify_results(output_dir):
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'kban.or.kr' in content:
                        url_check_passed += 1
            except Exception as e:
    ...
    # JSP 기반 사이트 성공률 분석
    if total_items > 0:
        print(f"✅ JSP 기반 사이트 파싱 성공: {total_items}개 공고 추출")
    else:
        print(f"❌ JSP 파싱 실패: 공고를 찾을 수 없음")
        ...
        if os.path.exists(content_file):
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    ...
                    if any(keyword in content for keyword in venture_keywords):
                        keyword_found += 1
            except:
                pass
    
    if keyword_found > 0:
        keyword_rate = (keyword_found / total_items) * 100
        print(f"벤처투자 관련 콘텐츠: {keyword_found}개 ({keyword_rate:.1f}%)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='KBAN 스크래퍼 테스트')
    ...
    try:
        success = test_kban_scraper(pages)
