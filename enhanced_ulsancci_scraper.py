# -*- coding: utf-8 -*-
"""
UlsanCCI(울산상공회의소) 스크래퍼 - Enhanced 버전
"""
...
class EnhancedUlsanCCIScraper(StandardTableScraper):
    """울산상공회의소 공지사항 스크래퍼 - 향상된 버전"""
...
# 테스트용 함수
def test_ulsancci_scraper(pages=3):
    """UlsanCCI 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedUlsanCCIScraper()
    output_dir = "output/ulsancci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"UlsanCCI 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("UlsanCCI 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_ulsancci_scraper(3)