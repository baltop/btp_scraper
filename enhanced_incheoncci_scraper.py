# -*- coding: utf-8 -*-
"""
IncheonCCI(인천상공회의소) 스크래퍼 - Enhanced 버전
"""
...
class EnhancedIncheonCCIScraper(StandardTableScraper):
    """인천상공회의소 공지사항 스크래퍼 - 향상된 버전"""
    
    def __init__(self):
        super().__init__()
        self.base_url = "https://incheon.korcham.net"
        self.list_url = "https://incheon.korcham.net/front/board/boardContentsListPage.do?boardId=51228&menuId=10130"
    ...
    def parse_list_page(self, html_content: str) -> list:
        ...
        for i, row in enumerate(rows):
            try:
                ...
                # 상세 페이지 URL 구성
                detail_url = f"{self.base_url}/front/board/boardContentsView.do?boardId=51228&menuId=10130&contentsId={announcement_id}"
                ...
                logger.info(f"공고 추가: {title} (ID: {announcement_id})")
            ...
            for i, attachment in enumerate(detail['attachments'], 1):
                try:
                    ...
                    if self.download_file(file_url, file_path):
                        downloaded_count += 1
                        logger.info(f"첨부파일 다운로드 완료: {safe_filename}")
                    else:
...
# 테스트용 함수
def test_incheoncci_scraper(pages=3):
    """IncheonCCI 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedIncheonCCIScraper()
    output_dir = "output/incheoncci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"IncheonCCI 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("IncheonCCI 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_incheoncci_scraper(3)