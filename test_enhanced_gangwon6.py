...
# -*- coding: utf-8 -*-
"""
Enhanced Gangwon6 스크래퍼 테스트 스크립트
"""
...
from enhanced_gangwon6_scraper import EnhancedGangwon6Scraper
...
def test_gangwon6_scraper(pages=3):
    """Gangwon6 스크래퍼 테스트"""
    print(f"=== 강원6차산업지원센터 스크래퍼 테스트 시작 ({pages}페이지) ===")
    
    # 출력 디렉토리 설정
    output_dir = "output/gangwon6"
    print(f"출력 디렉토리: {output_dir}")
    
    # 스크래퍼 초기화
    scraper = EnhancedGangwon6Scraper()
...
def verify_results(output_dir):
    ...
    for folder_name in announcement_folders:
        ...
        if os.path.exists(content_file):
            ...
            with open(content_file, 'r', encoding='utf-8') as f:
                content = f.read()
                if '**원본 URL**:' in content and 'gangwon6.co.kr' in content:
                    url_check_passed += 1
    ...
    if excel_files > 0:
        print(f"✅ Excel 파일 {excel_files}개 발견 - 양식/데이터 특성 확인")
    ...
    for folder_name in announcement_folders:
        ...
        if os.path.exists(content_file):
            with open(content_file, 'r', encoding='utf-8') as f:
                content = f.read()
                if 's3.ap-northeast-2.amazonaws.com' in content:
                    aws_s3_files += 1
    
    if aws_s3_files > 0:
        print(f"✅ AWS S3 호스팅 파일 {aws_s3_files}개 확인 - 클라우드 기반 파일 관리")
...
if __name__ == "__main__":
    ...
    # 명령행 인자 처리
    import argparse
    parser = argparse.ArgumentParser(description='Enhanced Gangwon6 스크래퍼 테스트')
    ...
    if args.single:
        # 단일 공고 테스트
        test_gangwon6_scraper(pages=1)
    else:
        # 전체 테스트
        test_gangwon6_scraper(pages=args.pages)