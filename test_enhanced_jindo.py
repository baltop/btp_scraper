...
import sys
import logging
from enhanced_jindo_scraper import EnhancedJindoScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('jindo_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_jindo_scraper(pages=3):
    """진도군 스크래퍼 테스트 - 기본값 3페이지"""
    output_dir = "output/jindo"  # 표준 출력 디렉토리
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"진도군 스크래퍼 테스트 시작 - {pages}페이지")
    
    try:
        scraper = EnhancedJindoScraper()
        ...
        if os.path.exists(content_file):
            ...
            try:
                with open(content_file, 'r', encoding='utf-8') as f:
                    content = f.read()
                    if '**원본 URL**:' in content and 'jindo.go.kr' in content:
                        url_check_passed += 1
            except Exception as e:
...
def test_single_page():
    ...
    test_jindo_scraper(pages=1)

def test_three_pages():
    """3페이지 테스트"""
    logger.info("3페이지 테스트 시작")
    test_jindo_scraper(pages=3)

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='진도군 스크래퍼 테스트')
