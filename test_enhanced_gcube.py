...
from enhanced_gcube_scraper import EnhancedGCUBEScraper

def test_gcube_scraper(pages=3):
    """GCUBE 스크래퍼 테스트"""
    print("=" * 60)
    print("GCUBE 향상된 스크래퍼 테스트 시작")
    print("=" * 60)
    
    try:
        scraper = EnhancedGCUBEScraper()
        
        # 출력 디렉토리 설정
        output_dir = "output/gcube"
...
def single_page_test():
    """단일 페이지 테스트"""
    print("🔬 단일 페이지 테스트 실행...")
    return test_gcube_scraper(pages=1)

def full_test():
    """전체 3페이지 테스트"""
    print("🚀 전체 3페이지 테스트 실행...")
    return test_gcube_scraper(pages=3)

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='GCUBE 스크래퍼 테스트')
    ...
    if args.single:
        ...
        success = test_gcube_scraper(args.pages)
