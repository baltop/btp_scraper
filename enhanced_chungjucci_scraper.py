# -*- coding: utf-8 -*-
"""
ChungjuCCI(충주상공회의소) 스크래퍼 - Enhanced 버전
"""
...
class EnhancedChungjuCCIScraper(StandardTableScraper):
    """충주상공회의소 공지사항 스크래퍼 - 향상된 버전"""
    
    def __init__(self):
        super().__init__()
        self.base_url = "https://chungjucci.korcham.net"
        self.list_url = "https://chungjucci.korcham.net/front/board/boardContentsListPage.do?boardId=10259&menuId=1051"
        ...
        self.detail_base_url = "https://chungjucci.korcham.net/front/board/boardContentsView.do"
    ...
    def parse_list_page(self, html_content: str) -> list:
        ...
        for row in data_rows:
            try:
                ...
                content_id = None
                if onclick:
                    # javascript:contentsView('115761') 형태에서 ID 추출
                ...
                # 상세 페이지 URL 구성
                detail_url = f"{self.detail_base_url}?contentsId={content_id}&boardId=10259&menuId=1051"
            ...
            with sync_playwright() as p:
                ...
                for i, row in enumerate(rows):
                    try:
                        ...
                        # 헤더 행인지 확인 - th가 있으면 건너뛰기
                        has_th = len(row.locator('th').all()) > 0
                        if has_th:
                            logger.debug(f"행 {i}: 헤더 행 건너뛰기")
                            continue
                        
                        # 충주상공회의소 구조: 제목(첫 번째), 조회수(두 번째)
                        title_cell = cells[0]
                        ...
                        views = ""
                        if len(cells) >= 2:
                            views = cells[1].inner_text().strip()
        ...
        try:
            ...
            with sync_playwright() as p:
                ...
                try:
                    ...
                    # 직접 URL로 접근 시도
                    direct_url = f"{self.detail_base_url}?contentsId={content_id}&boardId=10259&menuId=1051"
    ...
    def parse_detail_page(self, html_content: str) -> dict:
        ...
        if detail_table:
            ...
            for row in rows:
                ...
                # 본문 내용 추출 (셀이 1개이고 내용이 긴 경우)
                if len(cells) == 1:
                    cell_text = cells[0].get_text(strip=True)
                    if len(cell_text) > 100:  # 긴 내용만 본문으로 간주
...
# 테스트용 함수
def test_chungjucci_scraper(pages=3):
    """ChungjuCCI 스크래퍼 테스트"""
    import os
    ...
    scraper = EnhancedChungjuCCIScraper()
    output_dir = "output/chungjucci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"ChungjuCCI 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("ChungjuCCI 스크래퍼 테스트 완료")

if __name__ == "__main__":
    test_chungjucci_scraper(3)