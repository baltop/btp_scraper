...
# -*- coding: utf-8 -*-
"""
JBBA Enhanced 스크래퍼 테스트 스크립트
"""

import logging
import os
import sys
from enhanced_jbba_scraper import EnhancedJBBAScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('jbba_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_jbba_scraper(pages=3):
    """JBBA 스크래퍼 테스트"""
    logger.info("=== JBBA Enhanced 스크래퍼 테스트 시작 ===")
    
    try:
        # 스크래퍼 초기화
        scraper = EnhancedJBBAScraper()
        
        # 출력 디렉토리 설정 - output/jbba
...
def verify_results(output_dir):
    ...
    for folder_name in announcement_folders[:15]:  # 상위 15개만 검증
        ...
        if os.path.exists(content_file):
            ...
            with open(content_file, 'r', encoding='utf-8') as f:
                content = f.read()
                if '**원본 URL**:' in content and 'jbba.kr' in content:
                    url_check_passed += 1
    ...
    for folder_name in announcement_folders[:10]:
        ...
        if os.path.exists(content_file):
            with open(content_file, 'r', encoding='utf-8') as f:
                content = f.read()
                if 'D-' in content:
                    status_count['D-day'] = status_count.get('D-day', 0) + 1
                if '마감' in content:
                    status_count['마감'] = status_count.get('마감', 0) + 1
                if '담당자' in content:
                    status_count['담당자정보'] = status_count.get('담당자정보', 0) + 1
...
def test_single_page():
    ...
    test_jbba_scraper(pages=1)

def test_three_pages():
    """3페이지 테스트"""
    logger.info("=== 3페이지 테스트 ===")
    test_jbba_scraper(pages=3)

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description='JBBA Enhanced 스크래퍼 테스트')
    ...
    if args.single:
        ...
        test_jbba_scraper(pages=args.pages)