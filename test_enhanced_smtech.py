...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('smtech_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_smtech_scraper(pages=3):
    """SMTECH 스크래퍼 테스트"""
    logger.info("=== Enhanced SMTECH 스크래퍼 테스트 시작 ===")
    ...
    try:
            ...
            if detail_response:
                ...
                if detail_info['attachments']:
                    ...
                    for i, att in enumerate(detail_info['attachments'], 1):
                        ...
                        logger.info(f"     타입: {att['download_type']}")
                        if att['file_id']:
                            logger.info(f"     파일ID: {att['file_id']}")
                ...
                with open(content_path, 'w', encoding='utf-8') as f:
                    ...
                    f.write(f"**사업명**: {test_ann.get('business_name', '')}\\n\\n")
                    f.write(f"**접수기간**: {test_ann.get('reception_period', '')}\\n\\n")
                    f.write(f"**공고일**: {test_ann.get('announcement_date', '')}\\n\\n")
                    f.write(f"**상태**: {test_ann.get('status', '')}\\n\\n")
                    f.write("---\\n\\n")
                    f.write(detail_info['content'])
                
                logger.info(f"테스트 본문 저장: {content_path}")
                
                # 첫 번째 첨부파일만 테스트 다운로드 (직접 링크만)
                if detail_info['attachments']:
                    direct_attachments = [att for att in detail_info['attachments'] if att['download_type'] == 'direct']
                    if direct_attachments:
                        test_attachment = direct_attachments[0]
                        ...
                        logger.info("직접 다운로드 가능한 첨부파일 없음 (JavaScript 파일만 존재)")
            else:
        ...
        for folder in result_folders:
            ...
            if os.path.exists(content_file):
                ...
                # SMTECH 관련 내용이 있는지 확인
                if 'smtech.go.kr' in content or '중소기업기술정보진흥원' in content or 'SMTECH' in content or '중소벤처기업부' in content:
                    successful_items += 1
...
def verify_results(output_dir="output/smtech"):
    """결과 검증"""
    logger.info("=== 결과 검증 ===")
    ...
    for folder in folders:
        ...
        if os.path.exists(content_file):
            ...
            # 원본 URL 포함 확인
            if '**원본 URL**:' in content and 'smtech.go.kr' in content:
                url_check_passed += 1
...
if __name__ == "__main__":
    ...
    success = test_smtech_scraper(pages)
    
    if success:
        print("\\n✓ Enhanced SMTECH 스크래퍼 테스트 성공!")
        verify_results()
    else:
        print("\\n✗ Enhanced SMTECH 스크래퍼 테스트 실패!")
        sys.exit(1)