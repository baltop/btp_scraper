...
class EnhancedJcciScraper(StandardTableScraper):
    """정읍상공회의소 공지사항 스크래퍼 - 향상된 버전"""
    ...
    def parse_list_page(self, html_content: str) -> list:
        ...
        for row in data_rows:
            try:
                ...
                # 작성일은 정읍CCI에서는 목록에 없을 수 있음 (상세페이지에서 확인)
                date = ""
                if len(cells) > 2:
...
def test_jcci_scraper(pages=3):
    ...
    scraper = EnhancedJcciScraper()
    output_dir = "output/jcci"
    os.makedirs(output_dir, exist_ok=True)
    
    logger.info(f"Jcci 스크래퍼 테스트 시작 - {pages}페이지")
    scraper.scrape_pages(max_pages=pages, output_base=output_dir)
    logger.info("Jcci 스크래퍼 테스트 완료")
