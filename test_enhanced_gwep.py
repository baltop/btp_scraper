...
import sys
import logging
from enhanced_gwep_scraper import EnhancedGwepScraper
...
def test_gwep_scraper(pages=3):
    """GWEP 스크래퍼 테스트"""
    logger.info("=== GWEP 스크래퍼 테스트 시작 ===")
    
    # 스크래퍼 초기화
    scraper = EnhancedGwepScraper()
    # 테스트를 위해 중복 검사 비활성화
    scraper.enable_duplicate_check = False
    
    # 출력 디렉토리 설정
    output_dir = "output/gwep"
    os.makedirs(output_dir, exist_ok=True)
    
    # 스크래핑 실행
    try:
        scraper.scrape_pages(max_pages=pages, output_base=output_dir)
        logger.info("=== GWEP 스크래퍼 테스트 완료 ===")
...
def verify_results(output_dir):
    ...
    try:
            ...
            if os.path.exists(content_file):
                ...
                try:
                    with open(content_file, 'r', encoding='utf-8') as f:
                        content = f.read()
                        if '**원본 URL**:' in content and 'gwep.or.kr' in content:
                            url_check_passed += 1
        ...
        category_rate = (category_count / successful_items) * 100 if successful_items > 0 else 0
        ...
        logger.info(f"카테고리 정보 포함: {category_count} ({category_rate:.1f}%)")
        ...
        if category_rate >= 80:
            logger.info("✅ 카테고리 정보 추출 정상 작동")
        else:
            logger.warning("⚠️ 카테고리 정보 추출에 문제가 있습니다")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='GWEP 스크래퍼 테스트')
