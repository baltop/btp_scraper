...
# -*- coding: utf-8 -*-
"""
한국테크노파크진흥회 Enhanced 스크래퍼 테스트 스크립트
"""
...
from enhanced_technopark_scraper import EnhancedTechnoparkScraper
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('technopark_scraper.log', encoding='utf-8')
    ]
)

logger = logging.getLogger(__name__)

def test_technopark_scraper(pages=3):
    """한국테크노파크진흥회 스크래퍼 테스트"""
    output_dir = "output/technopark"
    
    logger.info("=" * 50)
    logger.info("한국테크노파크진흥회 Enhanced 스크래퍼 테스트 시작")
    logger.info("=" * 50)
    ...
    # 스크래퍼 초기화
    scraper = EnhancedTechnoparkScraper()
...
def verify_results(output_dir):
    ...
    # 처리된 제목 파일 확인
    processed_file = os.path.join(output_dir, 'processed_titles_enhancedtechnopark.json')
    ...
    for folder in folders[:5]:  # 최대 5개만 상세 검증
        ...
        if os.path.exists(content_file):
            ...
            try:
                ...
                # 원본 URL 포함 확인
                if '**원본 URL**:' in content and 'technopark.kr' in content:
                    url_check_passed += 1
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='한국테크노파크진흥회 Enhanced 스크래퍼 테스트')
    ...
    if args.single:
        ...
        test_technopark_scraper(args.pages)